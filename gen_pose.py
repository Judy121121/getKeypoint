...
default_det_config = (
    f'{default_mmdet_root}/mmdet/.mim/configs/faster_rcnn/'
...
default_pose_config = (
    f'{default_mmpose_root}/mmpose/.mim/configs/wholebody/2d_kpt_sview_rgb_img/topdown_heatmap/'
...
def parse_args():
    ...
    parser.add_argument("--config", default="configs/phoenix-2014t_s2g.yaml", type=str, help="Training configuration file (yaml).")
...
def main():
    set_seed(8)
    args = parse_args()
    #加载该配置文件只是为了得到数据集，与检测关键点无关
    cfg = load_config(args.config)
    cfg['device'] = torch.device('cuda')
    model_dir = cfg['training']['model_dir']
    os.makedirs(model_dir, exist_ok=True)
    global logger
    logger = make_logger(model_dir=model_dir)


    path = osp.join('/media/cv3/store1/postgraduate/y2023/WLF/NLA-SLR', 'keypoints')
    h, w = 256, 256
    os.makedirs(path, exist_ok=True)
    from modelling.Tokenizer import GlossTokenizer_S2G
    token=GlossTokenizer_S2G(cfg['model']['RecognitionNetwork']['GlossTokenizer'])
    dataloader, sampler = build_dataloader(cfg, args.split, gloss_tokenizer=token)
    ...
    outputs = {}
    save_inte =100
    for k, batch_data in enumerate(dataloader):
        frames = batch_data['recognition_inputs']['sgn_videos'][0].numpy().transpose(0,2,3,1)*255  #[T,H,W,3]
        frames = np.uint8(frames)
        frames = np.split(frames, frames.shape[0], axis=0)
        video_id = batch_data['name'][0]
        # video_id = 0
        ...
        assert pose_results.shape == (len(batch_data['recognition_inputs']['sgn_videos'][0]), 133, 3)
        ...
        if (k+1)%save_inte == 0:
            if args.start_end is None:
                fname = '{:s}_{:d}.pkl'.format(args.split, k)
                # fname = '{:s}_rank{:d}_{:d}.pkl'.format(args.split, cfg['local_rank'], k)
            else:
                fname = '{:s}_start{:d}_end{:d}_{:d}.pkl'.format(args.split,  args.start_end[0], args.start_end[1], k)
            print('save to '+fname)
            with open(os.path.join(path, fname), 'wb') as f:
                pickle.dump(outputs, f)
            outputs = {}
